(()=>{
 ...
 function customerStats(){
  ...
  names.forEach(name=>{const m=state.customers?.[name]||{};map.set(norm(name),{name,total:0,last:null,code:m.code||"",favorite:Boolean(m.favorite)});});
  (state.orders||[]).forEach(o=>{const k=norm(o.customer);if(!map.has(k)){const m=state.customers?.[o.customer]||{};map.set(k,{name:o.customer,total:0,last:null,code:m.code||"",favorite:Boolean(m.favorite)});}const s=map.get(k);s.total+=Object.values(o.items||{}).reduce((a,n)=>a+(Number(n)||0),0);if(!s.last||new Date(o.createdAt)>new Date(s.last))s.last=o.createdAt;});
  return [...map.values()];
 }
 function hide(){box.classList.add("hidden");box.innerHTML="";}
 function showSuggestions(){
  const q=norm(input.value);if(!q){hide();return;}
  const rows=customerStats().filter(s=>norm(s.name).includes(q)||norm(s.code).includes(q)).sort((a,b)=>{const ap=norm(a.name).startsWith(q)||norm(a.code).startsWith(q)?0:1,bp=norm(b.name).startsWith(q)||norm(b.code).startsWith(q)?0:1;return Number(b.favorite)-Number(a.favorite)||ap-bp||b.total-a.total||a.name.localeCompare(b.name,"sr");}).slice(0,10);
  box.innerHTML="";if(!rows.length){hide();return;}
  rows.forEach(s=>{const b=document.createElement("button");b.type="button";b.className="customer-suggestion";b.innerHTML=`<span><strong>${s.favorite?"⭐ ":""}${escapeHtml(s.name)}</strong><small>${s.code?`Šifra: ${escapeHtml(s.code)} · `:""}Ukupno trebovano: ${s.total}</small></span><span class="suggestion-last">${s.last?`Poslednja poseta<br><strong>${fmt(s.last)}</strong>`:"Bez prethodne posete"}</span>`;b.onclick=()=>{input.value=s.name;selectCustomer(s.name);hide();input.blur();};box.appendChild(b);});
  box.classList.remove("hidden");
 }
 input.addEventListener("input",showSuggestions);input.addEventListener("focus",showSuggestions);window.addEventListener("customers-changed",showSuggestions);document.addEventListener("click",e=>{if(!e.target.closest(".customer-search-wrap"))hide();});
 const originalRender=renderProductSettings;
 renderProductSettings=function(){originalRender();[...document.querySelectorAll("#productSettings .settings-row")].forEach((row,index)=>{const name=state.products[index];if(!name)return;const edit=document.createElement("button");edit.type="button";edit.textContent="✏️";edit.setAttribute("aria-label","Preimenuj");edit.onclick=()=>renameProduct(name);row.insertBefore(edit,row.children[1]);row.classList.add("product-setting-row");});};
