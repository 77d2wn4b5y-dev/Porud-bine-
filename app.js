...
function switchTab(tab){activeTab=tab;document.querySelectorAll(".tab").forEach(b=>b.classList.toggle("active",b.dataset.tab===tab));document.querySelectorAll(".view").forEach(view=>view.classList.remove("active"));$(tab+"View")?.classList.add("active");els.saveBar.classList.toggle("hidden",tab!=="order");if(tab==="history")renderHistory();}
function notifyProductsChanged(){window.dispatchEvent(new CustomEvent("products-changed"));}
function renderProductSettings(){
 els.productSettings.innerHTML="";
 state.products.forEach((name,index)=>{const row=document.createElement("div");row.className="settings-row";row.innerHTML=`<span>${escapeHtml(name)}</span><button type="button" aria-label="Izmeni">✏️</button><button type="button" aria-label="Gore">↑</button><button type="button" aria-label="Dole">↓</button><button type="button" class="danger" aria-label="Obriši">✕</button>`;const [edit,up,down,del]=row.querySelectorAll("button");edit.onclick=()=>renameProduct(name);up.disabled=index===0;down.disabled=index===state.products.length-1;up.onclick=()=>moveProduct(index,-1);down.onclick=()=>moveProduct(index,1);del.onclick=()=>deleteProduct(name);els.productSettings.appendChild(row);});
}
function moveProduct(index,delta){const target=index+delta;if(target<0||target>=state.products.length)return;[state.products[index],state.products[target]]=[state.products[target],state.products[index]];persist();renderProductSettings();renderProducts(captureDraft());notifyProductsChanged();}
function renameProduct(oldName){const newName=prompt("Novi naziv artikla:",oldName)?.trim();if(!newName||newName===oldName)return;if(state.products.some(p=>p!==oldName&&p.toLocaleLowerCase("sr")===newName.toLocaleLowerCase("sr"))){showToast("Artikal već postoji");return;}state.products[state.products.indexOf(oldName)]=newName;persist();window.dispatchEvent(new CustomEvent("product-renamed",{detail:{oldName,newName}}));renderProductSettings();renderProducts(captureDraft());showToast("Naziv artikla je izmenjen");}
function addProduct(){const name=els.newProductInput.value.trim();if(!name)return;if(state.products.some(p=>p.toLocaleLowerCase("sr")===name.toLocaleLowerCase("sr"))){showToast("Artikal već postoji");return;}state.products.push(name);els.newProductInput.value="";persist();renderProductSettings();renderProducts(captureDraft());notifyProductsChanged();showToast("Artikal je dodat");}
function deleteProduct(name){if(!confirm(`Obrisati artikal „${name}“ sa liste? Stare porudžbine ostaju sačuvane.`))return;state.products=state.products.filter(p=>p!==name);persist();renderProductSettings();renderProducts(captureDraft());notifyProductsChanged();showToast("Artikal je uklonjen");}
